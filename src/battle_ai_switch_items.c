...
u8 GetMostSuitableMonToSwitchInto(void)
{
    ...
    while (invalidMons != (1 << PARTY_SIZE) - 1) // All mons are invalid.
    {
        ...
        if (bestMonId != PARTY_SIZE)
        {
            ...
            if (invalidMons == ((1 << PARTY_SIZE) - 1) && !checkedAllMonForSEMoves)  // If we already checked all for a super effective move, then use the one with the best typing
            {
                invalidMons = 0;
...
static u8 GetAI_ItemType(u8 itemId, const u8 *itemEffect) // NOTE: should take u16 as item Id argument
{
    ...
    else if (itemEffect[0] & ITEM0_DIRE_HIT || itemEffect[1] != 0 /*X ITEMS*/)
